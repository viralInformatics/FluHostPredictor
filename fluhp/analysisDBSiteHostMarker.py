def analysisDBSiteHostMarker(file,dir,DBSite,type):
    if type=="hostMarker":
        print("analysisSite ...")
        import re
        ...
        for eachmaker in sorted(dicMerged2.items(), key=lambda d:int(d[0].split('&')[0])):
            ...
            for string in dicMerged2[eachmaker].split('<>')[1].split('+'):
                ...
                if re.match(DBSeq.split("|")[1],querySeq) or re.match(DBSeq.split("|")[1],deletionState):
                    ...
                    # outInfo = outInfo+"\t"+eachmaker+querySeq+"\tAvian\t"+ reference+"\n"
                    s=1
                elif re.match(DBSeq.split("|")[0],querySeq)  or re.match(DBSeq.split("|")[0],deletionState):
                    # print(eachmaker,querySeq,DBSeq,"Non Virulent",reference)
                    outInfo = outInfo+"\t"+eachmaker+querySeq+ "\tHuman\t"  + reference+"\n"
                # elif DBSeq.split("|")[1]=="other":
                    # print(eachmaker,querySeq,DBSeq,"Virulent",reference)
                    # outInfo = outInfo + "\t" + eachmaker + querySeq + "\tVirulent\t" + reference + "\n"
                # else:
