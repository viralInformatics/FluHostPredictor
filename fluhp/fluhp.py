...
def ivew_task(resultFileDir, tempFileDir, inputFilePath, updateFastaDir):
    ...
    # print(querySeqFile, querySeqType, DBDir, dataBaseName, queryEValue, outfmt, outName, querySeqFileDir,tempFileDir)
    blastSeq(querySeqFile, querySeqType, DBDir, dataBaseName, queryEValue, outfmt, outName, querySeqFileDir,
             tempFileDir)
    # print(outName, tempFileDir)
...
def annotate_fasta_file(input_fasta_path, output_directory = ".", prefix = "", evalue = 1e-5,
                        update_file = True, threads = 20, suppress_output = False):
    """
...
def check_fasta_for_non_standard_amino_acids(input_file):
    """
    检查FASTA文件中的序列是否包含非标准氨基酸，并去除包含信号肽的序列中的信号肽部分。
    :param input_file: FASTA文件的路径。
    :return: 包含标准氨基酸的序列列表。
    """
...
def generate_combinations(group):
    ...
    生成组合，
    把combination-combination_214_0,158G,PB2,PB2-158E&PA-295L,
    combination-combination_214_0,158A,PB2,PB2-158E&PA-295L,
    combination-combination_214_1,295P,PA,PB2-158E&PA-295L,
    变成
    'combination-combination_214': [{'PB2': '158G', 'PA': '295P'}, {'PB2': '158A', 'PA': '295P'}]
    Parameters:
...
def process_protein_sites(df, protein_col, site_col):
    """
        情况 1：处理类型为 'combination' 的行
        1. 用 & 分割位点列。
        2. 使用正则表达式提取蛋白质类型和位点，确保正确识别复杂类型如 PB1-F2 和 PA-X,对于删除位点也做了处理。
        3. 先按照蛋白质类型排序，再按照位点数字进行排序。
        4. 重新组合排序后的部分。
        1）能够正确提取蛋白
        2）能够正确提取数字和对应残基
        对于H3-HA1-XXX这种也能很好的处理，因为在排序时将 H3-HA2 视为 H3 进行排序，但在组合时保持原始格式。

        情况 2：处理非 'combination' 类型且包含 HA1 和 & 的行或者 HA2 和 & 的行，
        这种只来源一个蛋白，无需蛋白排序，且一定是HX-HA1/2-数字+氨基酸残基，也就是前面的HX一定存在
        1. 使用正则表达式提取所有 HA1 和 HA2 的部分。
        2. 按位点数字排序 HA1 和 HA2 部分。
        3. 重新组合排序后的部分。

        情况 3：处理其他非 'combination' 类型的行
        1. 提取所有数字和对应的字母。
        2. 按数字排序。
        3. 重新组合排序后的部分。
...
def should_continue(pro, dic):
    ...
    This function checks if the 'pro' string contains the "combination" keyword. If it does,
    it then checks if any types from predefined HA_TYPES or NA_TYPES are present in 'pro'.
    Based on the type found, it verifies if the keys in the dictionary 'dic' match the expected
    keys ("H3" for HA_TYPES and "N2" for NA_TYPES).

    Parameters:
    pro (str): The property string to be checked for specific types and keywords.
    dic (dict): Dictionary with actual keys to be validated against expected keys based on the types found in 'pro'.

    Returns:
    bool: True if the processing should continue (i.e., if the keys do not match the expected keys
    when a type is found), otherwise False.
    """
...
def process_row(row):
    ...
    Parameters:
    row (dict): A dictionary representing a row of data with keys such as 'Protein Type',
                'Original Site', and 'Original protein type'.

    Returns:
    dict: A dictionary with additional keys 'params' and 'name' representing the processed
          parameters and original protein type, respectively.
    """
    map_dic = {}
...
def parse_args():
    parser = argparse.ArgumentParser(
        prog = 'fluhp',
        description = 'A comprehensive tool for influenza marker analysis and predictive modeling',
        epilog = 'Unlock advanced insights into influenza strains with fluhp.'
    )
    ...
    def add_prediction_parser(subparsers, parser_name, name, description):
        pred_parser = subparsers.add_parser(
            parser_name,
            help = f'Predict {description} using advanced machine learning models',
